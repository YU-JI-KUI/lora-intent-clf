...
# 命令行覆盖参数
uv run python src/train.py \
    --model_name_or_path /mnt/llm-ehpc-vo1633083-ro/llm_model/Qwen3-8B \
    --lora_rank 32 \
    --learning_rate 2e-4 \
    --num_train_epochs 10
```
...
| 参数 | 默认值 | 对应 YAML |
|------|--------|-----------|
| `--model_name_or_path` | `/mnt/llm-ehpc-vo1633083-ro/llm_model/Qwen3-8B` | `model_name_or_path` |
...
| 模型 | template |
|------|----------|
| /mnt/llm-ehpc-vo1633083-ro/llm_model/Qwen3-8B (当前) | qwen |
...
|------|-----------|-------------|--------|
| 基座模型 | `model_name_or_path` | `model_name_or_path` | `/mnt/llm-ehpc-vo1633083-ro/llm_model/Qwen3-8B` |
...
**Q: 如何使用挂载或本地路径的模型？**
A: 将 `model_name_or_path` 改为本地或挂载的绝对路径即可，如本项目使用的 ANS 挂载路径 `/mnt/llm-ehpc-vo1633083-ro/llm_model/Qwen3-8B`。LlamaFactory 和 PEFT 对路径和 Hub ID 的处理逻辑完全一致。
